...
def test_get_course_by_id_exists():
    response = client.get("/courses/64efb7dbb4d2237554d035ad")
    assert response.status_code == 200
    course = response.json()
    # get the course from the database
    course_db = db.courses.find_one({'_id': ObjectId('64efb7dbb4d2237554d035ad')})
...
def test_get_course_by_id_not_exists():
    response = client.get("/courses/64efb7dbb4d2237554d035ac")
    assert response.status_code == 404
    assert response.json() == {'detail': 'Course not found'}

def test_get_chapter_info():
    response = client.get("/courses/64efb7dbb4d2237554d035ad/1")
    assert response.status_code == 200
...
def test_get_chapter_info_not_exists():
    response = client.get("/courses/64efb7dbb4d2237554d035ad/20")
    assert response.status_code == 404
    assert response.json() == {'detail': 'Chapter not found'}

def test_rate_chapter():
    course_id = '64efb7dbb4d2237554d035ad'
    chapter_id = '1'
    rating = 1
